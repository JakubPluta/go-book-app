...
func TestContentTypeJson(t *testing.T){
	...
	middleware.ContentTypeJson(http.HandlerFunc(sampleHandlerFunc())).ServeHTTP(recorder, r)


	response := recorder.Result()

	if responseBody := recorder.Body.String(); responseBody != expRespBody {
		 t.Errorf("Wrong response body:  got %v want %v ", responseBody, expRespBody)
	}
